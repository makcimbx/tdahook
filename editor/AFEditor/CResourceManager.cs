    ...
    public class CResourceManager
    {
        ...
        public void SavePackage(string filename)
        {
            ...
            foreach (KeyValuePair<UInt32, PackageText> kv in dialogue)
            {
                ...
                else
                {
                    ...
                    byte[] encoded = Encoding.GetEncoding("UTF-8").GetBytes(dehumanized);
        ...
        public CResourceManager(string filename)
        {
            ...
            for (uint i = 0; i < textCount; i++)
            {
                ...
                pkg.sourceString = Encoding.GetEncoding("UTF-8").GetString(pkg.source);
                pkg.sourceString = Humanize(pkg.sourceString);

                if (translatedLen > 1)
                {
                    pkg.translation = Encoding.GetEncoding("UTF-8").GetString(reader.ReadBytes((int) translatedLen - 1));
